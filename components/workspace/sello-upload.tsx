...
export function SelloUpload({ initialSelloUrl }: Props) {
  const utils = trpc.useUtils()
  const updateSello = trpc.doctor.updateSello.useMutation({
    onSuccess: () => utils.doctor.myProfile.invalidate(),
  })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-semibold text-white">Sello y Firma (Doctor)</h2>
      </div>

      <UploadField
        label="Sello Digital / Firma"
        endpoint="/api/upload/sello"
        initialUrl={initialSelloUrl}
        maxSizeMb={2}
        hint="Aparecerá en la parte inferior derecha de recetas, órdenes médicas y PDF de consultas (fondo transparente recomendado)."
        onSaved={(url) => updateSello.mutate({ selloUrl: url || undefined })}
      />
    </div>
  )
}
