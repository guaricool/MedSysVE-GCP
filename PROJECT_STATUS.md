...
| 2026-07-20 | `pending` | **Portal EHR integration & Security hardening** — Portal login block for unverified users with WhatsApp/Email verification direct panel. Unified local Patient profiles under global profile via cedula/email/phone match. Blood type, allergy list, and medical history exposed in portal profile screen. Remediated msmtprc credential exposure, upgraded dependencies to secure versions (`nodemailer@9.0.3` and `postcss@8.5.15` overrides), and hardened GitHub Actions workflow permissions. |
