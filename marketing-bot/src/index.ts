...
import { generateContentForImage, generateAIImage } from "./services/ai";
...
async function main() {
  ...
  try {
    ...
    const igUser = await getSecret("IG_SYSTEM_USER");
    const igPass = await getSecret("IG_SYSTEM_PASS");
    ...
    if (postStyle === "hyperrealistic") {
      ...
      const { buffer, moduleDescription } = await takeScreenshot(igUser, igPass);
      imageBuffer = buffer;
    ...
    const { mediaId: postId, url: imageUrl } = await publishToInstagram(
      imageBuffer, 
      caption, 
      hashtags, 
      igToken, 
