...
> **Coolify dashboard:** `http://13.140.181.29:8000/`
...
| **Coolify** | `cpierluissis@gmail.com` | `<coolify-admin-password>` | http://13.140.181.29:8000/login |
| **PostgreSQL** | `medsysve` | `<db-password>` | host: `tf03dm49her0vco2lprdqbjm:5432`, db: `medsysve` (solo accesible desde dentro del VPS) |

> **Importante:** Las passwords reales NO viven en este documento (audit de seguridad #3 — ver git log). Para reset operacional, contactar al dev owner o rotar desde Coolify UI / psql directo.
...
| DB | PostgreSQL 16 (Coolify container) | Running |
| Cache | Redis 7 (Coolify container) | Running |
| Reverse proxy | Coolify v4.1.2 / Traefik | OK |
| Build pack | Dockerfile | OK |
| Dominio | `*.13.140.181.29.sslip.io` (wildcard sslip.io) | OK |

**Importante:** el FS de Coolify es **efímero**. Los archivos en `public/uploads/` se borran al reiniciar el contenedor. Por eso **todos los PDFs se generan on-demand** desde la DB (no se guardan en disco).
...
2. Coolify detecta el push (webhook) y rebuildea automáticamente
...
### Deploy manual desde Coolify UI

1. Coolify → Project `MedSysVE` → Environment `production` → Application `medsysve`
2. Click en **"Redeploy"** (header de la app)
3. Esperar ~2-3 min para build + deploy

### Revertir a una versión anterior

1. Coolify → Application `medsysve` → tab **"Rollback"**
2. Seleccionar el commit/tag al que quieres volver
3. Confirmar

### Migraciones Prisma: drift P3009 (ALTER TABLE pre-deploy)

**Síntoma**: el deploy de Coolify revienta con `prisma migrate deploy` corriendo `P3009: failed to migrate database; the database is not empty` o `drift detected: migration X was modified after it was applied`.
...
**Caso real**: 2026-07-02 — Carlos aplicó `ALTER TABLE "AuditEvent" ADD COLUMN "archivedAt" TIMESTAMP(3)` directamente con `psql` para desbloquear el deploy del audit #10, pero no actualizó `_prisma_migrations`. El deploy de Coolify no podía avanzar porque Prisma asumía que la migration era nueva y se quejaba de drift.
...
2. **Push de un retrigger vacío** (`git commit --allow-empty -m "chore: retrigger Coolify deploy after migration resolve"`) para forzar al webhook a reintentar el deploy sobre el mismo SHA. Esta es la técnica usada en `8b66522`.

**Workaround moderno (preferido a futuro)**:

Una vez que Coolify ya levantó la app con la migration aplicada, podemos normalizar desde la consola del container:
...
Hay 2 cron jobs definidos pero no están corriendo. Configurar con un cron externo (ej: `cron-job.org`, `EasyCron`, o un cron en Coolify):
...
El `CRON_SECRET` está en las env vars de Coolify.

### Env vars requeridas (Coolify → app `medsysve` → Environment)

| Variable | Tipo | Generar con | Notas |
|---|---|---|---|
| `DATABASE_URL` | URL | (auto desde Coolify DB) | Connection string de Postgres. Si rota, actualizar aquí Y correr `ALTER USER` en Postgres. Ver sección "Drift P3009" arriba. |
...
**Verificación rápida** (desde VPS, sin app):
```bash
...
| 2 | Pre/Post deployment commands vacíos en Coolify | Media | **Arreglado**: pre-deployment ahora es `npx prisma migrate deploy` |
...
| 5 | Docs dicen Twilio pero código usa Meta Cloud API | Baja | **Arreglado**: actualizados `.env.example`, `coolify/docker-compose.yml`, `SISTEMA.md`, `docs/MEMORIA-SISTEMA.md`. |
...
**Causa probable:** El contenedor app está corriendo pero el proxy de Coolify no enrutó, o el contenedor crasheó al startup.

**Diagnóstico:**
1. Coolify → Application `medsysve` → tab **"Logs"**
...
**Diagnóstico:**
1. Coolify → Databases → `medsysve-db` → tab **"Logs"**
